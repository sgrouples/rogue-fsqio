...
trait IndexChecker {
  ...
  /** Verifies that the indexes expected for a query actually exist in the mongo
    * database. Logs an error via {@link QueryLogger#logIndexMismatch} if there
    * is no matching index. This version of validaateIndexExpectations is
    * intended for use in cases where the indexes are not explicitly declared in
    * the class, but the caller knows what set of indexes are actually
    * available.
    * @param query
    ...
    */
  def validateIndexExpectations(
      query: Query[_, _, _],
      indexes: Seq[UntypedMongoIndex]
  ): Boolean

  /** Verifies that the index expected by a query both exists, and will be used
    * by MongoDB to execute that query. (Due to vagaries of the MongoDB
    * implementation, sometimes a conceptually usable index won't be found.)
    * @param query
    *   the query
    * @param indexes
    *   the list of indexes that exist in the database
    */
  def validateQueryMatchesSomeIndex(
      query: Query[_, _, _],
      indexes: Seq[UntypedMongoIndex]
  ): Boolean
}

/** A utility object which provides the capability to verify if the set of
  * indexes that actually exist for a MongoDB collection match the indexes that
  * are expected by a query.
  */
object MongoIndexChecker extends IndexChecker {

  /** Flattens an arbitrary query into DNF - that is, into a list of query
    * alternatives implicitly joined by logical "or", where each of alternatives
    * consists of a list of query clauses implicitly joined by "and".
    */
  def flattenCondition(
      condition: MongoHelpers.AndCondition
  ): List[List[QueryClause[_]]] = {
  ...
  def normalizeCondition(
      condition: MongoHelpers.AndCondition
  ): List[List[QueryClause[_]]] = {
    flattenCondition(condition).map(
      _.filter(_.expectedIndexBehavior != DocumentScan)
    )
  }
  ...
  /** Verifies that the indexes expected for a query actually exist in the mongo
    ...
    *   true if the required indexes are found, false otherwise.
    */
  override def validateIndexExpectations(
      query: Query[_, _, _],
      indexes: Seq[UntypedMongoIndex]
  ): Boolean = {
    val baseConditions = normalizeCondition(query.condition);
    val conditions =
      baseConditions.map(_.filter(_.expectedIndexBehavior != DocumentScan))
      ...
      clauses.forall(clause => {
        ...
        badExpectations.forall {
          case (expectation, badActual) => {
            if (
              clause.expectedIndexBehavior == expectation &&
  ...
  /** Verifies that the index expected by a query both exists, and will be used
    ...
    *   the list of indexes that exist in the database
    */
  override def validateQueryMatchesSomeIndex(
      query: Query[_, _, _],
      indexes: Seq[UntypedMongoIndex]
  ) = {
    val conditions = normalizeCondition(query.condition)
    lazy val indexString =
      indexes.map(idx => "{%s}".format(idx.toString())).mkString(", ")
    conditions.forall(clauses => {
      clauses.isEmpty || matchesUniqueIndex(clauses) ||
      indexes.exists(idx =>
        matchesIndex(
          idx.asBsonDocument.keySet.asScala.toList,
          clauses
        ) && logIndexHit(query, idx)
      ) ||
      signalError(
        query,
        "Query does not match an index! query: %s, indexes: %s" format (query.toString, indexString)
      )
    })
  }
  ...
  private def matchesUniqueIndex(clauses: List[QueryClause[_]]) = {
    ...
    clauses.exists(clause =>
      clause.fieldName == "_id" && clause.actualIndexBehavior == Index
    )
  }
  ...
  /** Matches a compound index against a list of query clauses, verifying that
    ...
    * @param clauses
    *   a list of query clauses joined by logical "and".
    * @return
    *   true if every clause of the query is matched by a field of the index;
    *   false otherwise.
    */
  private def matchesCompoundIndex(
    ...
    if (clauses.isEmpty) {
      ...
      index match {
        ...
        case field :: rest => {
          val (matchingClauses, remainingClauses) =
            clauses.partition(_.fieldName == field)
          matchingClauses match {
            case matchingClause :: _ => {
              // If a previous field caused a scan, this field must scan too.
              val expectationOk =
                !scanning || matchingClause.expectedIndexBehavior == IndexScan
              ...
              expectationOk && matchesCompoundIndex(
                rest,
                remainingClauses,
  ...
  /** Utility method that allows us to signal an error from inside of a
    * disjunctive expression. e.g., "blah || blah || black ||
    * signalError(....)".
    *
    * @param query
    *   the query involved
