...
/*
  ...
  inline final def summonFormats[T <: Tuple]: Array[MacroBsonFormat[_]] = summonFormatsRec[T].toArray

  inline final def summonBsonFormat[A]: MacroBsonFormat[A] = summonFrom {
    case bsonFormat: MacroBsonFormat[A] => bsonFormat
    case _: Mirror.Of[A]     => MacroBsonFormat.derived[A] //AsObject.derived
  }
  ...
  inline final def summonFormatsRec[T <: Tuple]: List[MacroBsonFormat[_]] =
    inline erasedValue[T] match {
      case _: EmptyTuple => Nil
      case _: (t *: ts)  => summonBsonFormat[t] :: summonFormatsRec[ts]
    }
}


private[macros] trait MacroBsonFormatDerivation {
  inline final def derived[A](using inline A: Mirror.Of[A]): MacroBsonFormat.AsObject[A] =
    new MacroBsonFormat[A]
      with DerivedInstance[A]( //
      constValue[A.MirroredLabel],
      Derivation.summonLabels[A.MirroredElemLabels]
    ) {
      protected[this] lazy val elemFormats: Array[MacroBsonFormat[_]] =
        Derivation.summonFormats[A.MirroredElemTypes]

      final def encodeObject(a: A): BsonDocument = inline A match {
...
private[macros] trait DerivedInstance[A](
...
/*
private[macros] trait EncoderDerivation {
...
private[macros] trait DecoderDerivation {
...
private[macros] trait CodecDerivation {
...
private[macros] trait DerivedDecoder[A] extends DerivedInstance[A] with Decoder[A] {
