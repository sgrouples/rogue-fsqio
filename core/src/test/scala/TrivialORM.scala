...
case class SimpleARecord(a: Int, b: String) extends TrivialORM.Record {
  override type Self = SimpleARecord

  override def meta: SimpleARecord.type = SimpleARecord
}

object SimpleARecord extends TrivialORM.Meta[SimpleARecord] {
  val a = new OptionalField[Int, SimpleARecord.type] {
    override val owner = SimpleARecord
    override val name = "a"
  }
  val b = new OptionalField[String, SimpleARecord.type] {
    override val owner = SimpleARecord
    override val name = "b"
  }
  ...
  override def toDBObject(record: SimpleARecord): DBObject = {
    (BasicDBObjectBuilder
      .start
      .add(a.name, record.a)
      .add(b.name, record.b)
  ...
  override def fromDocument(doc: Document): SimpleARecord = {
    new SimpleARecord(doc.get(a.name).asInstanceOf[Int], doc.get(b.name).asInstanceOf[String])
  }

  override def toDocument(record: SimpleARecord): Document = {
    new Document(a.name, record.a).append(b.name, record.b)
  }
}