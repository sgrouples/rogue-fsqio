...
sealed trait Sh
    extends ShardKeyNotSpecified
    with ShardKeySpecified
    with AllShardsOk
...
object AddNaturalOrder {
  implicit def addOrder[Rest >: Sel with Lim with Sk with Or with Sh]
      : AddNaturalOrder[Rest with Unordered, Rest with Ordered] = null
}
...
object AddSelect {
  implicit def addSelect[Rest >: Ord with Lim with Sk with Or with Hn with Sh]
      : AddSelect[
        Rest with Unselected,
        Rest with Selected,
        Rest with SelectedOne
      ] = null
}
...
object AddOrClause {
  implicit def addOrClause[
      Rest >: Ord with Sel with Lim with Sk with Hn with Sh
...
trait AddShardAware[-In, +Specified, +AllOk]
    extends Required[In, ShardKeyNotSpecified]
object AddShardAware {
  implicit def addShardAware[
      Rest >: Ord with Sel with Lim with Sk with Or with Hn
  ]: AddShardAware[
    Rest with ShardKeyNotSpecified,
    Rest with ShardKeySpecified,
    Rest with AllShardsOk
  ] = null
}

@implicitNotFound(
  msg = "In order to call this method, ${A} must NOT be a subclass of ${B}."
)
sealed trait !<:<[A, B]
object !<:< {
  implicit def any[A, B]: A !<:< B = null
  implicit def intentionallyAmbiguousImplicitForNotSubclassTypeConstraint[
      A,
      B >: A
  ]: A !<:< B = null
  implicit def intentionallyAmbiguousImplicitForNotSubclassTypeConstraint2[
      A,
      B >: A
  ]: A !<:< B = null
}
...
class Required[-A, +B] {
  def apply[M, R](q: Query[M, R, A]): Query[M, R, B] =
    q.asInstanceOf[Query[M, R, B]]
}
object Required {
  val default = new Required[Any, Any]
  implicit def conforms[A]: Required[A, A] =
    default.asInstanceOf[Required[A, A]]
}
...
object ShardingOk {
  ...
  implicit def unsharded[M, State](implicit
      ev: M !<:< Sharded
  ): ShardingOk[M, State] = null
}

@implicitNotFound(
  msg =
    "${M} is a sharded collection. Either specify the shard key or use `.updateMulti()`."
)
trait RequireShardKey[M, -S]
object RequireShardKey {
  implicit def sharded[M <: Sharded, Sh <: ShardKeySpecified]
      : RequireShardKey[M, Sh] = null
  implicit def unsharded[M, State](implicit
      ev: M !<:< Sharded
  ): RequireShardKey[M, State] = null
}
