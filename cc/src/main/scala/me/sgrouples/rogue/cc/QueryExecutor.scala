...
trait RogueBsonSerializer[From, To]
    extends RogueBsonRead[To]
    with RogueBsonWrite[From]
...
trait AsyncBsonQueryExecutor[MB] extends ReadWriteSerializers[MB] with Rogue {
  ...
  )(implicit ev: ShardingOk[M, State], dba: MongoDatabase): Future[Long] = {
  ...
  def exists[M <: MB, State](
      query: Query[M, _, State],
      readPreference: Option[ReadPreference] = None
  )(implicit ev: ShardingOk[M, State], dba: MongoDatabase): Future[Boolean] = {
  ...
  def distinct[M <: MB, V: ClassTag, State](
      ...
      field: M => Field[V, M]
  )(implicit ev: ShardingOk[M, State], dba: MongoDatabase): Future[Seq[V]] = {
  ...
  def fetch[M <: MB, R, State](
      ...
      readPreference: Option[ReadPreference] = None
  )(implicit ev: ShardingOk[M, State], dba: MongoDatabase): Future[Seq[R]] = {
  ...
  def foreach[M <: MB, R, State](
      ...
      f: R => Unit
  )(implicit ev: ShardingOk[M, State], dba: MongoDatabase): Future[Unit] = {
    val s = readSerializer[M, R](query.meta, query.select)
  ...
  def replaceOne[M <: MB, R](query: Query[M, R, _], r: R, upsert: Boolean)(
