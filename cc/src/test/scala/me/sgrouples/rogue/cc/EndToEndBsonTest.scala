...
class EndToEndBsonTest extends FunSuite {
  ...
  override def beforeAll():Unit = {
    dbOpt = Some(MongoTestConn.connectToMongo().getDatabase("e2e-macro-async").withCodecRegistry(CcMongo.codecRegistry))
  }
...
assertEquals(    ccMetaToQueryBuilder(VenueR).where(_.id eqs v._id).fetch().map(_._id), Seq(v._id))
assertEquals(    VenueR.where(_.mayor eqs v.mayor).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.mayor eqs v.mayor).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.venuename eqs v.venuename).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.closed eqs false).fetch().map(_._id), List(v._id))
...
assertEquals(    VenueR.where(_.mayor_count neqs 5).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.mayor_count < 5).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.mayor_count lt 5).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.mayor_count <= 5).fetch().map(_._id), List(v._id))
assertEquals(    VenueR.where(_.mayor_count lte 5).fetch().map(_._id), List(v._id))
...
assertEquals(    VenueR.where(_.mayor_count between (3, 5)).fetch().map(_._id), List(v._id))
assertEquals(    VenueClaimR.where(_.status neqs ClaimStatus.approved).fetch().map(_._id), Nil)
assertEquals(    VenueClaimR.where(_.status neqs ClaimStatus.pending).fetch().map(_._id), List(vc._id))
  }
...
assertEquals(  base.select(_.legacyid, _.userId).fetch(), List((v.legId, v.userId)))
assertEquals(    base.select(_.legacyid, _.userId, _.mayor).fetch(), List((v.legId, v.userId, v.mayor)))
assertEquals(    base.select(_.legacyid, _.userId, _.mayor, _.mayor_count).fetch(), List((v.legId, v.userId, v.mayor, v.mayor_count)))
assertEquals(    base.select(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed).fetch(), List((v.legId, v.userId, v.mayor, v.mayor_count, v.closed)))
assertEquals(    base.select(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, _.tags).fetch(), List((v.legId, v.userId, v.mayor, v.mayor_count, v.closed, v.tags)))
  }

  test("selectEnum") {
    val v = baseTestVenue()
    VenueR.insertOne(v)
assertEquals(    VenueR.where(_.id eqs v._id).select(_.status).fetch(), List(VenueStatus.open))
  }
...
assertEquals(    base.selectCase(_.legacyid, _.userId, V2).fetch(), List(V2(v.legId, v.userId)))
assertEquals(    base.selectCase(_.legacyid, _.userId, _.mayor, V3).fetch(), List(V3(v.legId, v.userId, v.mayor)))
assertEquals(    base.selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, V4).fetch(), List(V4(v.legId, v.userId, v.mayor, v.mayor_count)))
assertEquals(    base.selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, V5).fetch(), List(V5(v.legId, v.userId, v.mayor, v.mayor_count, v.closed)))
assertEquals(    base.selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, _.tags, V6).fetch(), List(V6(v.legId, v.userId, v.mayor, v.mayor_count, v.closed, v.tags)))
  }
...
assertEquals(    VenueR.where(_.id eqs v._id).select(_.lastClaim.subfield(_.uid)).fetch(), List(None))
assertEquals(    VenueR.where(_.id eqs v._id).select(_.lastClaim.subselect(_.uid)).fetch(), List(None))
assertEquals(    VenueR.where(_.id eqs v._id).select(_.claims.subselect(_.uid)).fetch(), List(None))
  }
  /*
...
assertEquals(    v4.map(_.userId), Some(7L))
  ...
  test("RegexQuery") {
    val v = baseTestVenue()
    VenueR.insertOne(v)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename startsWith "test v").count(), 1L)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename matches ".es. v".r).count(), 1L)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename matches "Tes. v".r).count(), 0L)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename matches Pattern.compile("Tes. v", Pattern.CASE_INSENSITIVE)).count(), 1L)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename matches "test .*".r).and(_.legacyid in List(v.legId)).count(), 1L)
assertEquals(    VenueR.where(_.id eqs v._id).and(_.venuename matches "test .*".r).and(_.legacyid nin List(v.legId)).count(), 0L)
