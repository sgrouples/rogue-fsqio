...
class MacroQueryTest extends JUnitMustMatchers {
  ...
  */
  ...
  def testModifyQueryShouldProduceACorrectJSONQueryString {
    ...
    VenueR.where(_.legacyid eqs 1).modify(_.claims setTo claims).toString() must_== query + """{ "$set" : { "claims" : [ { "uid" : 1, "status" : "Approved" , "source" : { "name" : "" , "url" : ""} , "date" : { "$date" : "2010-05-01T00:00:00.000Z"} }]} }""" + suffix

    //query + """{ "$set" : { "claims" : [ { "status" : "Approved" , "uid" : 1, "source" : { "name" : "" , "url" : ""} , "date" : { "$date" : "2010-05-01T00:00:00.000Z"} }]} }""" + suffix
    VenueR.where(_.legacyid eqs 1).modify(_.lastClaim setTo claims.head).toString() must_== query + """{ "$set" : { "lastClaim" : { "uid" : 1, "status" : "Approved" , "source" : { "name" : "" , "url" : ""} , "date" : { "$date" : "2010-05-01T00:00:00.000Z"} }} }""" + suffix
  ...
  */
  ...
  def testOrQueryShouldProduceACorrectJSONQueryString {
    ...
    VenueR.or(
      ...
      .toString() must_== """db.venues.find({ "$or" : [ { "legId" : { "$numberLong" : "1" } } , { "mayor" : 2 }] })"""
    ...
    VenueR.where(_.tags size 0)
      .or(
        _.where(_.legacyid eqs 1),
        _.where(_.mayor eqs 2))
      .toString() must_== """db.venues.find({ "tags" : { "$size" : 0 } , "$or" : [ { "legId" : { "$numberLong" : "1" } } , { "mayor" : 2 }] })"""
    ...
    VenueR.where(_.tags size 0)
      .or(
        ...
        _.where(_.mayor eqs 2))
      .toString() must_== """db.venues.find({ "tags" : { "$size" : 0 } , "$or" : [ { "legId" : { "$numberLong" : "1" }, "closed" : true} , { "mayor" : 2 }] })"""
    ...
    VenueR.or(
      ...
      _.where(_.mayor eqs 2))
      .toString() must_== """db.venues.find({ "$or" : [ { "legId" : { "$numberLong" : "1" }, "$or" : [ { "closed" : true} , { "closed" : { "$exists" : false} }]} , { "mayor" : 2 }] })"""

    // $or with modify
    VenueR.or(
      _.where(_.legacyid eqs 1),
      _.where(_.mayor eqs 2))
      .modify(_.userId setTo 1).toString() must_== """db.venues.update({ "$or" : [ { "legId" : { "$numberLong" : "1" } } , { "mayor" : 2 }] }, { "$set" : { "userId" : 1 } }, false, false)"""

    // $or with optional where clause
    VenueR.or(
      _.where(_.legacyid eqs 1),
      _.whereOpt(None)(_.mayor eqs _))
      .modify(_.userId setTo 1).toString() must_== """db.venues.update({ "$or" : [ { "legId" : { "$numberLong" : "1" } }]}, { "$set" : { "userId" : 1 } }, false, false)"""
