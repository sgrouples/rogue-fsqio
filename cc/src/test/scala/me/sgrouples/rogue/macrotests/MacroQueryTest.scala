...
class MacroQueryTest extends JUnitMustMatchers {
  ...
  */
    ...
    VenueR.where(_.mayor eqs 1).select(_.legacyid, _.userId, _.mayor).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).select(_.legacyid, _.userId, _.mayor, _.mayor_count).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).select(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "closed": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).select(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, _.tags).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "closed": 1, "tags": 1, "_id": 0})"""

    // select case queries
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, V1).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, _.userId, V2).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, _.userId, _.mayor, V3).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, V4).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, V5).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "closed": 1, "_id": 0})"""
    VenueR.where(_.mayor eqs 1).selectCase(_.legacyid, _.userId, _.mayor, _.mayor_count, _.closed, _.tags, V6).toString() must_== """db.venues.find({"mayor": {"$numberLong": "1"}}, {"legId": 1, "userId": 1, "mayor": 1, "mayor_count": 1, "closed": 1, "tags": 1, "_id": 0})"""

    // select subfields
    TipR.where(_.legacyid eqs 1).select(_.counts at "foo").toString() must_== """db.tips.find({"legid": {"$numberLong": "1"}}, {"counts.foo": 1, "_id": 0})"""
    ...
    VenueR.where(_.legacyid eqs 1).select(_.lastClaim.subselect(_.status)).toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}}, {"lastClaim.status": 1, "_id": 0})"""
    VenueR.where(_.legacyid eqs 1).select(_.claims.subselect(_.uid)).toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}}, {"claims.uid": 1, "_id": 0})"""

    // select slice
    VenueR.where(_.legacyid eqs 1).select(_.tags).toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}}, {"tags": 1, "_id": 0})"""
    VenueR.where(_.legacyid eqs 1).select(_.tags.slice(4)).toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}}, {"tags": {"$slice": 4}, "_id": 0})"""
    VenueR.where(_.legacyid eqs 1).select(_.tags.slice(4, 7)).toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}}, {"tags": {"$slice": [4, 7]}, "_id": 0})"""
  ...
  def testModifyQueryShouldProduceACorrectJSONQueryString: Unit = {
    ...
    VenueR.where(_.legacyid eqs 1).modify(_.claims setTo claims).toString() must_== query + """{"$set": {"claims": [{"uid": {"$numberLong": "1"}, "status": "Approved", "source": {"name": "", "url": ""}, "date": {"$date": 1272672000000}}]}}""" + suffix

    //query + """{"$set": {"claims": [ {"status": "Approved", "uid": 1, "source": {"name": "", "url": ""}, "date": {"$date": "2010-05-01T00:00:00.000Z"}}]}}""" + suffix
    VenueR.where(_.legacyid eqs 1).modify(_.lastClaim setTo claims.head).toString() must_== query + """{"$set": {"lastClaim": {"uid": {"$numberLong": "1"}, "status": "Approved", "source": {"name": "", "url": ""}, "date": {"$date": 1272672000000}}}}""" + suffix
  ...
  */
  ...
  def testDollarSelector: Unit = {
    ...
    VenueR.where(_.legacyid eqs 1)
      .and(_.tags contains "sometag")
      .select(_.tags.$$)
      .toString() must_== """db.venues.find({"legId": {"$numberLong": "1"}, "tags": "sometag"}, {"tags.$": 1, "_id": 0})"""
  }
  /*
        @Test
        def testWhereOpt {
