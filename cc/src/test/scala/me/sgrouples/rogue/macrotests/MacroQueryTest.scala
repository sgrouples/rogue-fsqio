...
//assertEquals(    VenueR.where(_.legacyid eqs 1).select(_.geolatlng.unsafeField[Double]("lat")).q, pq("""db.venues.find({"legId": {"$numberLong": "1"}}, {"latlng.lat": 1})"""))
    ...
    assertEquals(
      VenueR
        ...
        .and(
          _.claims elemMatch (_.status eqs ClaimStatus.approved, _.uid gt 2097)
        )
        .q,
      pq(
...
//assertEquals(    VenueR.where(_.legacyid eqs 1).modify(_.geolatlng setTo ll).q, pq(query + """{"$set": {"latlng": [ 37.4, -73.9]}}""" + suffix))
    ...
    assertEquals(
      ...
      pq(
        query + """{"$set": {"claims": [{"uid": {"$numberLong": "1"}, "status": "Approved", "source": {"name": "", "url": ""}, "date": {"$date": 1272672000000}}]}}""" + suffix
      )
    )
    ...
    assertEquals(
      ...
      pq(
        query + """{"$set": {"lastClaim": {"uid": {"$numberLong": "1"}, "status": "Approved", "source": {"name": "", "url": ""}, "date": {"$date": 1272672000000}}}}""" + suffix
      )
    )
    ...
    assertEquals(
      VenueR
        ...
        .q,
      pq(
        query + """{"$inc": {"mayor_count": 1}, "$set": {"mayor_count": {"$numberLong": "3"}, "venuename": "fshq"}}""" + suffix
      )
    )
    assertEquals(
      VenueR
    ...
    assertEquals(
      VenueR.where(_.legacyid eqs 1).modify(_.popularity pullWhere (_ gt 2)).q,
      pq(
    ...
    assertEquals(
      VenueR
        .where(_.legacyid eqs 1)
        .modify(
          _.claims pullObjectWhere (_.status eqs ClaimStatus.approved, _.uid eqs 2097)
        )
        .q,
      pq(
...
assertEquals(      Comment.where(_.comments.unsafeField[String]("comment") contains "hi").signature(), """db.comments.find({"comments.comment": 0})""")
  ...
  test("WhereOpt") {
    ...
    assertEquals(
      VenueR.whereOpt(someId)(_.legacyid eqs _).q,
      pq("""db.venues.find({"legId": {"$numberLong":"1"}})""")
    )
    assertEquals(
      VenueR.whereOpt(noId)(_.legacyid eqs _).q,
      pq("""db.venues.find({})""")
    )
    assertEquals(
      VenueR.whereOpt(someId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq(
        """db.venues.find({"legId": {"$numberLong":"1"}, "mayor": {"$numberLong":"2"}})"""
      )
    )
    assertEquals(
      VenueR.whereOpt(noId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq("""db.venues.find({"mayor": {"$numberLong":"2"}})""")
    )

    // whereOpt: lists
    assertEquals(
      VenueR.whereOpt(someList)(_.legacyid in _).q,
      pq(
        """db.venues.find({"legId": {"$in": [ {"$numberLong":"1"}, {"$numberLong":"2"}]}})"""
      )
    )
    assertEquals(
      VenueR.whereOpt(noList)(_.legacyid in _).q,
      pq("""db.venues.find({})""")
    )
    ...
    assertEquals(
      VenueR.whereOpt(someEnum)(_.status eqs _).q,
      pq("""db.venues.find({"status": "Open"})""")
    )
    assertEquals(
      VenueR.whereOpt(noEnum)(_.status eqs _).q,
      pq("""db.venues.find({})""")
    )
...
//assertEquals(          VenueR.whereOpt(noDate)(_.last_updated after _).q, pq("""db.venues.find({})"""))
    ...
    // andOpt
    assertEquals(
      VenueR.where(_.mayor eqs 2).andOpt(someId)(_.legacyid eqs _).q,
      pq(
        """db.venues.find({"mayor": {"$numberLong":"2"} , "legId": {"$numberLong":"1"}})"""
      )
    )
    assertEquals(
      VenueR.where(_.mayor eqs 2).andOpt(noId)(_.legacyid eqs _).q,
      pq("""db.venues.find({"mayor": {"$numberLong":"2"}})""")
    )

    // scanOpt
    assertEquals(
      VenueR.scanOpt(someId)(_.legacyid eqs _).q,
      pq("""db.venues.find({"legId": {"$numberLong":"1"}})""")
    )
    assertEquals(
      VenueR.scanOpt(noId)(_.legacyid eqs _).q,
      pq("""db.venues.find({})""")
    )
    assertEquals(
      VenueR.scanOpt(someId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq(
        """db.venues.find({"legId": {"$numberLong":"1"}, "mayor": {"$numberLong":"2"}})"""
      )
    )
    assertEquals(
      VenueR.scanOpt(noId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq("""db.venues.find({"mayor": {"$numberLong":"2"}})""")
    )

    // iscanOpt
    assertEquals(
      VenueR.iscanOpt(someId)(_.legacyid eqs _).q,
      pq("""db.venues.find({"legId": {"$numberLong":"1"}})""")
    )
    assertEquals(
      VenueR.iscanOpt(noId)(_.legacyid eqs _).q,
      pq("""db.venues.find({})""")
    )
    assertEquals(
      VenueR.iscanOpt(someId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq(
        """db.venues.find({"legId": {"$numberLong":"1"}, "mayor": {"$numberLong":"2"}})"""
      )
    )
    assertEquals(
      VenueR.iscanOpt(noId)(_.legacyid eqs _).and(_.mayor eqs 2).q,
      pq("""db.venues.find({"mayor": {"$numberLong":"2"}})""")
    )
    ...
    assertEquals(
      q.modifyOpt(someId)(_.legacyid setTo _).q,
      pq(prefix + """{"$set": {"legId": {"$numberLong":"1"}}}""" + suffix)
    )
    assertEquals(
      q.modifyOpt(noId)(_.legacyid setTo _).q,
      pq(prefix + """{}""" + suffix)
    )
    assertEquals(
      q.modifyOpt(someEnum)(_.status setTo _).q,
      pq(prefix + """{"$set": {"status": "Open"}}""" + suffix)
    )
    assertEquals(
      q.modifyOpt(noEnum)(_.status setTo _).q,
      pq(prefix + """{}""" + suffix)
    )
  }
...
assertEquals(          LikeR.where(_.checkin eqs 1).withShardKey(_.userid eqs 123).q, pq("""db.likes.find({"checkin": 1, "userid": 123})"""))
  ...
  test("CommonSuperclassForPhantomTypes") {
    ...
    assertEquals(
      maybeLimit(1, Some(5)).q,
      pq("""db.venues.find({"legId": {"$numberLong":"1"}}).limit(5)""")
    )
  }
