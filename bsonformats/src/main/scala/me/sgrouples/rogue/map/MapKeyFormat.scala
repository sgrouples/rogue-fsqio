...
object MapKeyFormat {
  ...
  def apply[T](
    reader: String => T,
    writer: T => String = defaultWriter _): MapKeyFormat[T] = new DefaultMapKeyFormat(reader, writer)
}

class DefaultMapKeyFormat[T](
  val reader: String => T,
  val writer: T => String = MapKeyFormat.defaultWriter _) extends MapKeyFormat[T] {
