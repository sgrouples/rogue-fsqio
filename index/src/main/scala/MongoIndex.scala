...
case class MongoIndex1[R](f1: Field[_, R], m1: IndexModifier)
    extends MongoIndex[R] {
  def asBsonDocument: BsonDocument =
    new BsonDocument().append(f1.name, m1.bsonValue)
}
...
trait IndexBuilder[M] {
  def index(f1: Field[_, M], m1: IndexModifier): MongoIndex1[M] =
    MongoIndex1[M](f1, m1)
...
case class MongoTextIndexAll[R]() extends MongoTextIndex[R] {
  def asBsonDocument: BsonDocument =
    new BsonDocument().append("$**", new BsonString("text"))
}

case class MongoTextIndex1[R](f1: Field[_, R]) extends MongoTextIndex[R] {
  def asBsonDocument: BsonDocument =
    new BsonDocument().append(f1.name, new BsonString("text"))
}

case class MongoTextIndex2[R](f1: Field[_, R], f2: Field[_, R])
...
case class MongoTextIndex3[R](f1: Field[_, R], f2: Field[_, R], f3: Field[_, R])
    extends MongoTextIndex[R] {
  def asBsonDocument: BsonDocument = new BsonDocument()
    .append(f1.name, new BsonString("text"))
    .append(f2.name, new BsonString("text"))
...
case class TextIndexBuilder[M](rec: M) {
  ...
  def textIndex(f1: Field[_, M]): MongoTextIndex1[M] =
    MongoTextIndex1[M](f1)

  def textIndex(f1: Field[_, M], f2: Field[_, M]): MongoTextIndex2[M] =
